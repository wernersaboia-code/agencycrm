...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    try {
        ...
        // Uma entrada por lista POR idioma publicado, cada uma com hreflang
        // recíproco — mesmo padrão das rotas estáticas acima. Antes submetíamos
        // só a URL pt e canonicalizávamos as variantes para ela; a medição com
        // a URL Inspection API mostrou que o Google ignorava o canonical e
        // indexava 322 variantes mesmo assim, deixando 31 canônicas de fora.
        // Agora cada variante é anunciada como versão de idioma legítima (ver
        // generateMetadata em app/[locale]/list/[slug]/page.tsx).
        const listRoutes = lists.flatMap((list) => {
            const path = `/list/${list.slug}`
            const languages = alternatesFor(path).languages
